...
# parse out the single line, multiple token input
# literal approach has problems with later joins
#mea2.df = mea.df %>% rowwise() %>% mutate(mea=parseMetricTokens(MEA)$level)
#mea2.df = mea.df %>% rowwise() %>% mutate(level=parseMetricTokens(MEA)$level)
#mea3.df = mea2.df %>% rowwise() %>% mutate(count=parseMetricTokens(MEA)$count)
